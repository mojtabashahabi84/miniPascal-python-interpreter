...
class Lexer:
  ...
  def make_less_than(self):
    ...
    if tok_type == TT_LT: return Token(tok_type, '<',pos_start=pos_start, pos_end=self.pos)
    elif tok_type == TT_LTE: return Token(tok_type, '<=',pos_start=pos_start, pos_end=self.pos)
    elif tok_type == TT_NE: return Token(tok_type, '<>',pos_start=pos_start, pos_end=self.pos)
  ...
  def make_greater_than(self):
    ...
    if tok_type == TT_GT: return Token(tok_type, '>',pos_start=pos_start, pos_end=self.pos)
    elif tok_type == TT_GTE: return Token(tok_type, '>=',pos_start=pos_start, pos_end=self.pos)
...
class ForNode:
  ...
  #def __repr__(self):
    #return f'({"FOR "}, {self.var_name_tok}, {":="},{self.start_value_node}, {" TO "}, {self.end_value_node}, {"THEN"})'    
...
class FuncDefNode:
  ...
  #def __repr__(self):
    #return f'({"FUNCTION "},{self.var_name_tok}, (, {self.arg_name_toks}, ))'    
...
def run(fn, text):
  ...
  if tokens:
    ...
    tokenFile.write('{:20s}{:20s}\n'.format('LEXEME', 'SPELLING'))
    tokenFile.write(10*'-' + 10*' '+ 10*'-' + '\n')
    if len(tokens) == 1:
      tokenFile.write(repr(tokens[0]) + '\n')
    else:
      for tokken in tokens:
        wTokken = repr(tokken).split(":")
        if wTokken[1] in ["READ", "WRITE", "READ_INT"]:
          wTokken[0] = "KEYWORD"
        if wTokken[0] == "EQ":
          wTokken[1] = ":="        
          
        tokenFile.write("{:<20s}{:<20s}".format(wTokken[0], wTokken[1]) + '\n')
      tokenFile.close()

  elif error:
    tokenFile= open("Token.txt", "w")
    tokenFile.write(error.as_string())
    tokenFile.close()
    return None, error
  ...
  parserFile= open("Parser.txt", "w")
  try:
    if ast.error:
      parserFile.write(ast.error.as_string())

    else:
      parserFile.write(str(ast))
